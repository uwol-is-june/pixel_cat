...
- [x] VSCode `onDidChangeTextDocument` 또는 1분 인터벌 타이머로 활성 코딩 감지
  - 파일 편집 이벤트 발생 시 해당 분을 "활성"으로 카운트
  - 1분마다 체크 → XP +1 → globalState 업데이트 → webview에 전달
- [x] `/food`, `/pet` 커맨드 실행 시 XP +5 보너스
- [x] 레벨 계산 함수
  ```js
  ...
  ```
- [x] XP 변경 시 레벨 재계산 → 레벨업 감지 → 레벨업 이벤트 트리거

---

## Task 5 — 레벨업 연출

- [x] 레벨업 시 webview에 `levelUp` 메시지 전달
- [x] webview: 레벨업 파티클 폭발 (속성 파티클 대량 발생)
- [x] extension: VSCode 알림 메시지 표시
  ```
  🎉 Nabi가 Lv.3으로 성장했어요!
  ```
- [x] 레벨업 후 스프라이트 즉시 갱신
...
- [x] **Lv.1** — 현재 스프라이트 기반, 속성 팔레트만 적용. 눈 크고 머리 비율 큼
- [x] **Lv.2** — 전체 비율 소폭 성장, 줄무늬 1~2개 추가
- [x] **Lv.3** — 성묘 비율, 귀 상단에 털 픽셀 추가
- [x] **Lv.4** — 속성 악세서리 그리기
  - 불: 머리 위 작은 불꽃 왕관
  - 물: 목 부분 물방울 목걸이
  - 풀: 귀 사이 잎사귀 머리띠
- [x] **Lv.5** — 오라 파티클 상시 발생 + 특별 색상
  - 불: 몸통 금빛 (`#ffd700`)
  - 물: 몸통 은빛 (`#c0e8ff`)
  - 풀: 몸통 황금잎 (`#d4ff70`)
...
- [x] 상태바 텍스트에 속성 이모지 + 레벨 포함
  ```
  🔴 =^･ω･^= Lv.3
  ```
- [x] XP/레벨 변경 시 상태바 텍스트 즉시 갱신
- [x] 4프레임 애니메이션은 유지 (레벨·이모지 부분은 고정, 고양이 얼굴 부분만 변환)

---

## Task 8 — 데이터 복원 (재시작)

- [x] webview 로드 시 extension이 `globalState`의 `catType`·`xp`·`level` 전달
- [x] webview: 받은 데이터로 초기 상태 세팅 (알 선택 화면 vs 고양이 화면 분기)
- [x] 애니메이션 상태는 복원 안 해도 됨 — 항상 `sit`으로 시작

---

## Task 9 — 마켓플레이스 준비

- [x] README에 진화 단계 비교 스크린샷 추가
- [x] `CHANGELOG.md` 작성 (v0.1 ~ v0.2 변경 내역)
- [x] `vsce package` 빌드 테스트
- [x] 버전 번프 (0.1.5 → 0.2.0) 후 배포
